...
def _resolve_set_dirs(sets_path: Path, set_names: list[str] | None) -> list[Path]:
...
def _discover_pairs(sets_path: Path, set_names: list[str] | None) -> list[tuple[str, Path, Path | None]]:
...
def load_cvrp_dataset(
    ...
    n_amount: int | None = None,
    random_selection: bool = False,
    random_n_amount: bool = False,
    seed: int | None = None,
    verbose: bool = True,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load CVRP instances from nested set folders.")
    parser.add_argument(
        "--sets-dir",
        type=Path,
